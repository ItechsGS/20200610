...
public class HoloCircularProgressBar extends View {
    ...
    public boolean isBlackLineNeedToShow = true;
    /**
    ...
    private boolean isDashedCircle = false;
    ...
    public HoloCircularProgressBar(final Context context, final AttributeSet attrs,
        ...
        if (attributes != null) {
            try {
                ...
                isDashedCircle = attributes
                        .getBoolean(R.styleable.HoloCircularProgressBar_dashed_circle, false);
            } finally {
...
//                    (float) (mThumbPosX - mThumbRadius / 2 * 1.4), mThumbPosY, mMarkerColorPaint);
            canvas.drawLine(mThumbPosX + mThumbRadius / 4, mThumbPosY,
                    mThumbPosX - mThumbRadius / 4, mThumbPosY, mMarkerColorPaint);
            canvas.restore();
        }
    ...
    public void setProgress(final float progress) {
        ...
        if (isBlackLineNeedToShow == false) {
            updateMarkerColor();
            isBlackLineNeedToShow = true;
        }
    ...
    private void updateBackgroundColor() {
        ...
        if (isDashedCircle) {
            ...
            mBackgroundColorPaint.setPathEffect(dashPath);
        }
...
//        mMarkerColorPaint.setColor(mProgressBackgroundColor);
        if (isBlackLineNeedToShow)
            mMarkerColorPaint.setColor(Color.BLACK);
        else
            mMarkerColorPaint.setColor(Color.TRANSPARENT);
    ...
    private void updateProgressColor() {
        ...
        if (isDashedCircle) {
            DashPathEffect dashPath = new DashPathEffect(new float[]{5, 5}, (float) 1.0);
            mProgressColorPaint.setPathEffect(dashPath);
        }
