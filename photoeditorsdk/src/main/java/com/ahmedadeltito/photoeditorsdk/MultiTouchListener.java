...
class MultiTouchListener implements OnTouchListener {
    ...
    MultiTouchListener(View deleteView, RelativeLayout parentView,
                       ImageView photoEditImageView, OnPhotoEditorSDKListener onPhotoEditorSDKListener, Bitmap desiredImage) {
    ...
    public boolean onTouch(View view, MotionEvent event) {
        ...
        switch (action & event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                view.getLocationOnScreen(fieldImgXY);

                float eventX = fieldImgXY[0];
                float eventY = fieldImgXY[1];

                if (mCurrentCancelX == mPrevRawX || mCurrentCancelY == mPrevRawY) {
                    if (view instanceof TextView) {
                        if (onMultiTouchListener != null) {
                            onMultiTouchListener.onEditTextClickListener(
                                    ((TextView) view).getText().toString(), ((TextView) view).getCurrentTextColor(), null, eventX, eventY);
                        }
                    ...
                    if (view instanceof RelativeLayout) {
                        Log.e(TAG, "onTouch: " + ((RelativeLayout) view).getChildCount());
                        if (((RelativeLayout) view).getChildAt(1) instanceof TextView) {
                            if (onMultiTouchListener != null) {
                                onMultiTouchListener.onEditTextClickListener(
                                        ((TextView) ((RelativeLayout) view).getChildAt(1)).getText().toString(), ((TextView) ((RelativeLayout) view).getChildAt(1)).getCurrentTextColor(), desiredImage, eventX, eventY);
                            }
                            if (onPhotoEditorSDKListener != null) {
                                onPhotoEditorSDKListener.onEditTextChangeListener(
                                        ((TextView) ((RelativeLayout) view).getChildAt(1)).getText().toString(), ((TextView) ((RelativeLayout) view).getChildAt(1)).getCurrentTextColor(), desiredImage, eventX, eventY);
    ...
    interface OnMultiTouchListener {
        void onEditTextClickListener(String text, int colorCode, Bitmap drawable, float x, float y);
